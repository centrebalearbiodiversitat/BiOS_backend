...
class BaseTaxonLevelAdmin(admin.ModelAdmin):
	list_display = ['scientific_name', 'rank', 'upper_taxon', 'num_references', 'num_children', 'verbatim_authorship', 'parsed_year']
	list_filter = ['rank', 'accepted']
	fields = ['name', 'verbatim_authorship', 'rank', 'parent', 'accepted', 'synonyms', 'references', 'parsed_year', 'authorship']
	search_fields = ['unidecode_name', 'verbatim_authorship']
	autocomplete_fields = ['parent', 'synonyms', 'authorship']
	# exclude = ['references']
